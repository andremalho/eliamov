...
export class ActivitiesService {
  ...
  async createForUser(userId: string, dto: CreateActivityDto) {
    const shareToken = randomBytes(16).toString('hex');
    const entity = this.repo.create({ ...dto, userId, shareToken } as any);
    const activity = await this.repo.save(entity) as unknown as Activity;
    await this.challengesService.onActivityCreated(userId, activity);
    return activity;
  }
